...
import { LayoutTemplate, Trash2, Plus, Zap, Info, ShieldAlert, Link, Star, AlertTriangle, ArrowRight, Clock, Image as ImgIcon, Moon, Sun } from 'lucide-react';
...
const WidgetPreview = ({ w, subW }) => {
    ...
    if (w.type === 'clock') {
        return (
            <div className="w-full bg-[#0a0a0a] border border-blue-500/30 p-6 flex flex-col justify-center items-center h-40 rounded-[32px] shadow-[0_0_30px_-10px_rgba(59,130,246,0.2)]">
    ...
    if (w.type === 'image') {
        return (
            <div className="w-full relative overflow-hidden rounded-[32px] h-40 border border-white/10 shadow-2xl">
    ...
    return (
        <div className={`p-6 rounded-[32px] border flex flex-col justify-between h-40 shadow-lg relative overflow-hidden ${getGlow(w.color)} ${w.is_wide?'w-full':'w-[150px]'}`}>
            <div className="flex justify-between items-start z-10"><div className="p-3 bg-white/5 rounded-2xl border border-white/5"><Ico size={28}/></div>{w.link && <ArrowRight size={16} className="opacity-40"/>}</div>
...
export default function WidgetBuilder() {
    ...
    const [w, setW] = useState({ title: 'Заголовок', text: 'Текст', color: 'blue', icon: 'Zap', link: '', image: '', is_wide: false });
    ...
    return (
        <div className="min-h-screen bg-black text-white p-8 font-sans flex gap-10">
            ...
            <div className="w-[400px] bg-[#111] border border-[#333] rounded-3xl p-8 h-fit max-h-[90vh] overflow-y-auto shadow-2xl">
                <h2 className="text-2xl font-bold mb-6 flex items-center gap-3"><LayoutTemplate className="text-blue-500"/> Конструктор</h2>
                
                <div className="flex gap-2 mb-8 bg-[#222] p-1.5 rounded-xl">
                    {['standard', 'image', 'clock', 'group'].map(t => (
                        <button key={t} onClick={()=>setType(t)} className={`flex-1 py-2.5 rounded-lg text-[10px] font-bold uppercase transition ${type===t?'bg-blue-600 text-white shadow-lg':'text-gray-500 hover:text-white'}`}>{t}</button>
                    ))}
                </div>
            ...
            <div className="w-[360px] flex justify-center pt-10">
                <div className="bg-[#050505] border-[12px] border-[#222] rounded-[50px] w-full h-[700px] relative shadow-2xl overflow-hidden flex flex-col">
                    ...
                    <div className="flex-1 bg-[#000] pt-16 px-5 overflow-y-auto no-scrollbar">
                        <div className="text-[10px] font-bold text-gray-600 text-center uppercase tracking-[0.2em] mb-6">iPhone 15 Pro • Live Preview</div>
                    ...
                    {widgets.map(wd => (
                        <div key={wd.id} className="bg-[#111] p-5 rounded-2xl border border-[#333] flex justify-between items-center group hover:border-[#555] transition">
                            <div className="flex items-center gap-4">
                                <div className="text-2xl bg-[#222] w-12 h-12 flex items-center justify-center rounded-xl">{wd.type === 'clock' ? '⏰' : wd.type==='image' ? '🖼️' : wd.type==='group' ? '🔲' : '⚡'}</div>
                                <div>
