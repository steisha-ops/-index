...
function initDb() {
    db.serialize(() => {
        ...
        const schema = [
            ...
            `CREATE TABLE IF NOT EXISTS authors (id INTEGER PRIMARY KEY, name TEXT, handle TEXT, avatar TEXT, bio TEXT, is_verified INTEGER, password TEXT, access_level INTEGER DEFAULT 0)`,
...
app.get('/api/authors', get("SELECT * FROM authors"));
app.get('/api/settings', get("SELECT * FROM settings"));

app.get('/api/authors/:id', (req, res) => {
    db.get("SELECT * FROM authors WHERE id=?", [req.params.id], (e, a) => {
...
app.post('/api/author/set-password', (req, res) => {
    ...
    bcrypt.hash(password, saltRounds, (err, hash) => {
        ...
        db.run("UPDATE authors SET password = ?, access_level = 1 WHERE id = ?", [hash, author_id], function(err) {
...
app.post('/api/author/login', (req, res) => {
    ...
    db.get("SELECT * FROM authors WHERE handle = ?", [handle], (err, author) => {
